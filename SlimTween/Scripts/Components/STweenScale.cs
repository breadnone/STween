...
public class STweenScale : MonoBehaviour
{
    ...
    [field: SerializeField] public int pingpong { get; set; }
    ...
    public void RotateToTargetObject()
    {
        ...
        if (isGameObject)
        {
            if (target != null)
            {
                STween.scale(target, to, duration).setSpeed(speedPower)
            ...
            {
                STween.scale(rectTransform, to, duration).setSpeed(speedPower)
                .setLoop(loopCount)
            ...
            {
                STween.scale(visualElement.rootVisualElement, to, duration).setSpeed(speedPower)
                .setLoop(loopCount)
